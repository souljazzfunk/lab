...
// Claude Code のクラウド環境では Playwright 同梱の Chromium（/opt/pw-browsers/chromium）を使う
...
const browser = await puppeteer.launch({ executablePath: CHROME, headless: true,
  args: process.getuid?.() === 0 ? ['--no-sandbox'] : [] });  // root で動くコンテナでは sandbox を外す
const page = await browser.newPage();
let ng = 0, scanned = 0;
for (const f of files) {
  const svg = fs.readFileSync(f, 'utf8');
  await page.setContent(`<html><body style="margin:0;background:#faf9f5">${svg}</body></html>`);
