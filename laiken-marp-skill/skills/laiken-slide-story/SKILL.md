---
name: laiken-slide-story
description: らいけんが人前で話す登壇・勉強会・講義スライドのストーリーの組み方。5歳児でもわかる大原則、つかみ、中扉、段階的な開示、見出しの文体、締め方、尺の見積り。「登壇スライドを作って」「LTの資料」「勉強会の資料」「講義資料の構成を考えて」と言われたら、書き始める前に読む。
---
...
この型は、みのるん（[@minorun365](https://x.com/minorun365)）が数年分の自作デッキから抽出した型（[minorun-marp-skill](https://github.com/minorun365/minorun-marp-skill)）を、らいけん（X: [@thunder_bark](https://x.com/thunder_bark)）のスライド向けに改造したもの。AIエージェントにスライドを書かせると体裁は整うのに中身が薄くなりがちで、その原因の大半は下に挙げる規則のどれかを外していることにある。

## 大原則：5歳児でもわかるスライド

会場のスクリーンを一番後ろの席から見ている人が、**2秒で理解できる**ことがすべてに優先する。新しいスライドを書くときも、既存のスライドを直すときも、まずこの4つを満たす。下の規則とぶつかったら、こちらを取る。

1. **5歳児でもわかる言葉で書く**
   - 専門用語は日常語に言い換える。「セマンティック層」→「言葉の定義」、「フォールバック」→「ダメなら次へ」
   - どうしても固有名詞が要るときは、すぐ横に一言で意味を添える
   - 1文＝1メッセージ。文を接続詞でつながない
2. **大きな絵を使う**
   - 3つ以上の関係を説明するなら、箇条書きではなくSVGの図にする（`laiken-slide-figures`）
   - 図は画面の半分以上を占める大きさで描く。小さな図を隅に置かない
   - 図のノードは3〜4個まで。細かい格子やマトリクスは分割する
3. **文字は少なく**
   - 1スライド＝1メッセージ。箇条書きは4行まで（推奨2〜3行）
   - 1項目は20文字以内（本文48pxで折り返さない上限）。長い説明はスピーカーノートへ逃がす
   - 入りきらないときは、文字を削るか2枚に割る。フォントを縮めて詰め込まない
4. **フォントは大きく**
   - 本文は36pt（48px）。下限は24pt（32px）。表、図の補足、キャプション、出典の注記もこの下限を守る
   - これ未満のサイズを、CSSにもSVGにも書かない。例外はスピーカーノートとページ番号だけ（客席のスクリーンに映らない、または読ませない）

## らいけんのデッキの約束

- 呼称は「らいけん」。一人称の自己紹介スライドも同じ表記にする
- 表紙に `らいけん（X: @thunder_bark）` を入れる。公開するデッキは、表紙か最終ページに CC BY 4.0 の表記を置く。AIに作図や構成を手伝わせたら、その旨も添える（例：作図・構成協力 Claude）
- 聴衆はデータ分析（Tableauなど）の文脈の人が多い。例やお題はその文脈から選ぶ。ただし専門用語はそのまま使わず、大原則1で言い換える
- 長いURLや出典はスライドに置かず、巻末の参考資料スライドかスピーカーノートへ回す
- 表紙のタイトルは、らいけんのテーマ `laiken-light` の `top` クラスで組む（`laiken-slide-design`）
...
概念の定義から始めない。原作者（みのるん）の人気デッキ21本を調べたところ、定義から始まるものは1本もなかった。型は4つある。
...
| 項目 | 規則 |
|---|---|
| 1スライドの量 | 見出し1つと本文2〜4行。これ以上載せない |
| 見出し | 主張の一文。めくった瞬間に何の話か伝わること |
| 本文 | 箇条書き2〜3項目（最大4）、各20文字以内の1文。階層は1段。収まらなければ2スライドに割る |
| 強調 | 1スライドに1か所まで。デッキ全体では本文2〜3スライドに1か所 |
| 記号 | ダッシュ（――）と全角コロンの多用を避ける |
| 図 | 1スライドに1つだけ、大きく。同じ見出しで「図だけの1スライド」を作ってよい |
| 表 | 2〜3列の単純なもの。ヘッダ＋本文5行まで。超えるなら図にするか参考資料へ |
| 見出しの行数 | 1行に収める。折り返すなら言い換える |
| 二か国語（Both） | 1項目が日英の2行になる。箇条書きは3行まで、締めの一文を足すなら2行まで。英語は直訳せず、同じ主張を英語の短い一文で言う |
| スピーカーノート | 削った説明の逃がし先。Marpでは、ディレクティブ以外のHTMLコメント `<!-- 話すこと -->` が発表者ノートになる。コロンを含む `key: value` の形は避ける |
...
「…」で始まるどんでん返しの見出し（「…人生そんなに簡単じゃないんです」）は、上の表のとおり敬体で落とす型なので対象外。`tools/check-static.py` も同じ基準で見る。

### 書き終えたら、全見出しを並べて数える

不自然さは語彙ではなく均一さとして出る。1スライドずつ見ると自然な日本語なので、通しで並べるまで気づけない。`rg -n '^# ' deck.md` で一覧にして実測する。原作者の手作りデッキ2本（各57〜58スライド）の値は次のとおり。らいけんのデッキで実測したら置き換える。
...
ドキュメントの見出しルールをそのまま持ち込まない。READMEでは見出しを機能名の名詞に揃え、売り文句を消すのが正しい。スライドでは逆で、聞き手を乗せるのが仕事なので「簡単」「〜するだけ！」は正当な要素になる。READMEの見出しは機能を探すための名札で、スライドの見出しは聞き手を次の話へ連れていく一言である。
...
再演の案件では、直前の回の構成メモを読むまで構成を提案しない。却下された案とその理由は、たいてい次の回にもそのまま当てはまる。再演の依頼は「評判の良かったものをもう一度」なので、枠組みの作り替えは依頼の意図に反する。差を作るのは構成ではなく、事実の鮮度とスライド単位の入れ替えである。地方での開催は、聴衆の大半が初めて聞く前提で組む。
...
単価は客層と枠の長さで変わる。下は原作者（みのるん）の実測なので、らいけんの実績が取れたら置き換える。
...
前提の説明を飛ばせるかどうかで単価が大きく変わる。短い枠では前置きも言い直しも消えるので速くなる。総スライド数に一律の秒数を掛ける計算は、本文と中扉を混ぜるぶん必ず遅い側へ外れ、要らない削減を提案することになる。
...
1. 見出し1つと本文2〜4行を超えるスライド、20文字を超える項目、24pt未満の文字が無いか
2. 見出しが項目名になっていないか
3. 図が1スライドに2つ以上無いか
4. 強調が1スライドに2か所以上無いか。強調のあるスライドが本文の半分を超えていないか
5. 中扉の直後のスライドが、その問いに答えているか
6. 予告、まとめ、全体像の先出しが紛れていないか
7. 5歳児でもわかる言葉になっているか。専門用語が言い換えなしで残っていないか

見た目の検査は `laiken-slide-design` と `tools/` を使う。
