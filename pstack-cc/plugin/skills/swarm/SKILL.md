...
Spawn all N workers in one message with `subagent_type: general-purpose`, `isolation: "worktree"`, `run_in_background: true`, and the step 4 model, left unset for `auto` or `inherit-parent`. Omit `isolation` only when the worker must see the parent's working tree. Workers run on this machine (Claude Code has no Cursor cloud), so start at most about 5 at once and queue the rest.

When a worker must start from a non-default pushed branch, name it in the brief and have the worker check it out in its worktree first.
