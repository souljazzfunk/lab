...
Take the runners from the `architect runners` line in `~/.claude/pstack-models.md`, in place of the `arena runners` line. If the rule or that line is missing, use `opus`, `fable`, `sonnet`. Alias and rejected entries follow the runner rules in the **arena** skill's Phase A.
