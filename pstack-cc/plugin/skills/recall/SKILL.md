...
Transcripts live at `~/.claude/projects/<slug>/<session-id>.jsonl`, where `<slug>` is the workspace path with each "/" (and any other non-alphanumeric character) turned into "-" (so `/Users/you/proj` becomes `-Users-you-proj`). Every line is one JSON event; chat messages have `"type":"user"` or `"type":"assistant"`.
